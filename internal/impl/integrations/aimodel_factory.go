...
func (f *AIModelFactory) CreateModelIntegration(model *entities.Model, provider *entities.Provider, apiKey string) (interfaces.AIModelIntegration, error) {
	...
	case entities.ProviderOpenAI:
		return NewOpenAIIntegration(endpoint, apiKey, model.Name, f.toolRepo, f.logger)
	case entities.ProviderAnthropic:
		return NewAnthropicIntegration(endpoint, apiKey, model.Name, f.toolRepo, f.logger)
	case entities.ProviderXAI:
		return NewXAIIntegration(endpoint, apiKey, model.Name, f.toolRepo, f.logger)
	case entities.ProviderGoogle:
		return NewGoogleIntegration(endpoint, apiKey, model.Name, f.toolRepo, f.logger)
	case entities.ProviderDeepseek:
		return NewDeepseekIntegration(endpoint, apiKey, model.Name, f.toolRepo, f.logger)
	case entities.ProviderTogether:
		return NewTogetherIntegration(endpoint, apiKey, model.Name, f.toolRepo, f.logger)
	case entities.ProviderGroq:
		return NewGroqIntegration(endpoint, apiKey, model.Name, f.toolRepo, f.logger)
	case entities.ProviderMistral:
		return NewMistralIntegration(endpoint, apiKey, model.Name, f.toolRepo, f.logger)
	case entities.ProviderOllama:
		return NewOllamaIntegration(endpoint, apiKey, model.Name, f.toolRepo, f.logger)
	case entities.ProviderDrujensen:
		return NewDrujensenIntegration(endpoint, apiKey, model.Name, f.toolRepo, f.logger)
	case entities.ProviderGeneric:
		// For generic providers, use the OpenAI-compatible API
		return NewGenericIntegration(endpoint, apiKey, model.Name, f.toolRepo, f.logger)
	default:
